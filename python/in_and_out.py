...
while True:
    ...
    if len(travelers) < capacity and random.random() <= NEW_TRAVELER_PROB:
        if random.randint(0, 1) == 0:
            # Add to left side
            travelers.append([0, random.randint(5, WIDTH - 1 - 5), random.randint(STAY_IN_MIN, STAY_IN_MAX), 'L']) # [current_position, destination_col, time_to_live]
        else:
            # Add to right side
            travelers.append([WIDTH - 1, random.randint(5, WIDTH - 1 - 5), random.randint(STAY_IN_MIN, STAY_IN_MAX), 'R']) # [current_position, destination_col, time_to_live]

    # Print all travelers
    line = list(EMPTY_CHAR * WIDTH)
    for traveler in travelers:
        line[traveler[0]] = TRAVELER_CHAR
